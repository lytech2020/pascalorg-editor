...
实现进展（2026-07-22）：本地 SQLite 已落地 request/model attempt，以及 T2.7 的工具调用、场景版本变化和验证摘要三张最小审计表。当前工具审计只保存参数形状与数量，不保存参数值、完整场景、原始工具响应或大型 diff；`artifact_ref` 仅预留给后续私有 artifact 存储。本文后续表格仍是面向生产能力的目标模型，未落地字段不应被理解为当前已经支持。
