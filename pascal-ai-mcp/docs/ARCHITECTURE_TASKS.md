...
- 验证：`request-context.test.ts` 4 用例（伪造 body ids 被忽略且每次唯一、合法头采纳、畸形头替换、clientRequestId 合法性）；528 测试全过、pascal-ai-mcp check-types 干净（editor 侧 2 个既有 tsc 错误在 ImportDxfTool/catalog-items-fs，与本任务无关）。端到端冒烟（真实服务器 + 伪造 ids + 合法 trace 头）：响应 requestId 为服务端新铸、traceId=trace-e2e-12345678、clientRequestId 回显、AI 日志两行同 ids。代理层日志接线经代码审查未跑 Next dev 实测（前端无类型错误）。
